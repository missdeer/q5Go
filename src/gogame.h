...
private:
    ...
    game_state(game_state_manager *gm,
               int                 id,
               const go_board &    b,
               int                 move,
               int                 sgf_move,
...
public:
    ...
    game_state *add_child_edit_nochecks(const go_board &new_board, stone_color to_move, bool scored, add_mode am)
    {
        m_visual_ok      = false;
        int         code = scored ? -3 : -2;
        game_state *tmp  = m_manager->create_game_state(new_board, m_move_number + 1, m_sgf_movenum + 1, this, to_move, code, code, none);
    ...
    game_state *replace_child_edit(game_state *child, const go_board &new_board, stone_color to_move)
    {
        for (size_t i = 0; i < m_children.size(); i++)
            ...
            {
                game_state *tmp = m_manager->create_game_state(new_board, m_move_number + 1, m_sgf_movenum + 1, this, to_move, -2, -2, none);
                m_children[i]   = tmp;
                child->m_parent = tmp;
    ...
    game_state *add_child_move_nochecks(const go_board &new_board, stone_color to_move, int x, int y, add_mode am)
    {
        ...
        m_visual_ok              = false;
        game_state *tmp          = m_manager->create_game_state(new_board, m_move_number + 1, m_sgf_movenum + 1, this, next_to_move, x, y, to_move);
        return insert_child(tmp, am);
    }
    ...
    {
        m_visual_ok       = false;
        game_state *tmp   = m_manager->create_game_state(new_board, m_move_number + 1, m_sgf_movenum + 1, this, m_to_move == black ? white : black);
    ...
    void add_child_tree(game_state *other)
    {
        ...
        auto callback = [](game_state *st) -> bool {
            st->m_move_number = st->m_parent->m_move_number + 1;
            return true;
        };
        other->m_parent = this;
...
template<typename... ARGS> game_state *game_state_manager::create_game_state(ARGS &&... args)
{
...
extern void                          db_info_from_sgf(go_board &                  b,
                                                      sgf::node *                 n,
                                                      bool                        is_root,
                                                      sgf_errors &                errs,
                                                      bit_array &                 final_w,
...
public:
    game_record(int size, const game_info &info) : m_info(info)
    {
        m_root = create_game_state(size);
    }
    game_record(const go_board &b, stone_color to_move, const game_info &info) : m_info(info)
    {
        m_root = create_game_state(b, to_move);
    }
    game_record(const go_board &b, stone_color to_move, const game_info &info, const std::shared_ptr<const bit_array> &m) : m_info(info), m_mask(m)
    {
        m_root = create_game_state(b, to_move);
    }
    game_record(const game_record &other) : m_info(other.m_info), m_modified(other.m_modified), m_errors(other.m_errors), m_mask(other.m_mask)
    {
        m_root = create_game_state(*other.m_root, nullptr);
    }

    const game_info &info()
    {
        return m_info;
    }
    void set_info(game_info i)
    {
        m_info = std::move(i);
    }
