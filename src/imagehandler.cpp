...
void ImageHandler::init(int size)
{
    ...
    stonePixmaps.append(QPixmap::fromImage(ib, Qt::PreferDither | Qt::DiffuseAlphaDither | Qt::DiffuseDither));
...
void ImageHandler::rescale(int size)
{
    ...
    stonePixmaps[0].convertFromImage(ib, Qt::PreferDither | Qt::DiffuseAlphaDither | Qt::DiffuseDither);
    QImage gb(ib);
    ghostImage(&gb);
    ghostPixmaps[0].convertFromImage(gb, Qt::PreferDither | Qt::DiffuseAlphaDither | Qt::DiffuseDither);
    ...
    for (int i = 1; i <= WHITE_STONES_NB; i++)
    {
        ...
        paint_one_stone(iw1, true, size, i - 1);
        stonePixmaps[i].convertFromImage(iw1, Qt::PreferDither | Qt::DiffuseAlphaDither | Qt::DiffuseDither);
        QImage gw1(iw1);
        ghostImage(&gw1);
        ghostPixmaps[i].convertFromImage(gw1, Qt::PreferDither | Qt::DiffuseAlphaDither | Qt::DiffuseDither);
    }
    ...
    if (m_look > 1)
        ...
        is.fill(0);
    stonePixmaps[WHITE_STONES_NB + 1].convertFromImage(is, Qt::PreferDither | Qt::DiffuseAlphaDither | Qt::DiffuseDither);
#if 0
...
static std::vector<std::tuple<ImageHandler::t_params, ImageHandler::t_params, int, bool>> presets = {{{95, 38, 1, 16}, {95, 61, 1, 0}, 15, true},
