...
db_dir_entry::db_dir_entry(const QString &s) : title(s), qdb_found(false), kdb_found(false)
{
    QDir d(s);
...
QVariant db_dir_entry::icon(int col) const
{
    if (col == 0)
    {
        return QIcon(qdb_found ? ":/images/green-check.png" : kdb_found ? ":/images/yellow-exclam.png" : ":/images/red-x.png");
    }
    return QVariant();
}
...
/* Create engine.  */
EngineDialog::EngineDialog(QWidget *parent) : QDialog(parent), ui(new Ui::EngineDialog)
{
    ui->setupUi(this);
    init();
}
...
EngineDialog::EngineDialog(QWidget *parent, const Engine &e, bool dup) : QDialog(parent), ui(new Ui::EngineDialog)
{
    ui->setupUi(this);
    if (dup)
    {
...
{
    Engine e(ui->engineName->text(),
             ui->enginePath->text(),
             ui->engineArgs->text(),
             ui->engineKomi->text(),
             ui->engineAnalysis->isChecked(),
             ui->engineSize->text(),
...
void EngineDialog::accept()
{
    ...
    {
        QMessageBox::warning(
            this, tr("No name set for the engine."), tr("A name must be set for the engine.\nPlease enter all necessary fields before continuing"));
        return;
    }
    if (ui->enginePath->text().isEmpty())
    {
        QMessageBox::warning(
            this, tr("No path set for the engine."), tr("A path must be set for the engine.\nPlease enter all necessary fields before continuing"));
        return;
    }
    int     p    = 0;
    ...
    {
        QMessageBox::warning(
            this, tr("Invalid size specified"), tr("The value entered for the board size is invalid.\nPlease enter values between 3 and 25."));
        return;
    }
    if (ui->engineAnalysis->isChecked() && ui->engineRestrictions->isChecked() && size.isEmpty())
    {
        QMessageBox::warning(
            this, tr("No boardsize specified for analysis engine."), tr("Uncheck the \"Restrictions\" field or specify a board size."));
        return;
    }
    QString komi = ui->engineKomi->text();
    if (!komi.isEmpty() && m_komi_vald.validate(komi, p) != QValidator::Acceptable)
    {
        QMessageBox::warning(this, tr("Invalid komi specified"), tr("The value entered for komi is invalid."));
        return;
    }
    QDialog::accept();
}
...
template<class T> void pref_vec_model<T>::update_entry(const QModelIndex &i, const T &v)
{
    int row = i.row();
    if (row >= 0)
    {
...
template<class T> bool pref_vec_model<T>::add_no_replace(T x)
{
    for (size_t i = 0; i < m_entries.size(); i++)
    {
        T &e = m_entries[i];
...
std::vector<Host> standard_servers {{"-- IGS --", "igs.joyjoy.net", 7777, "guest", "", "SJIS", -1},
...
PreferencesDialog::PreferencesDialog(int tab, QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::PreferencesDialogGui)
    , m_hosts_model(setting->m_hosts, standard_servers)
    , m_engines_model(setting->m_engines)
    , m_dbpath_model(setting->m_dbpaths)
{
    ...
    connect(ui->ambientSlider, &QSlider::valueChanged, [=](int) {
        update_w_stones();
        update_b_stones();
    });
    connect(ui->shadowSlider, &QSlider::valueChanged, [=](int) {
        update_w_stones();
        update_b_stones();
    });
    ...
    connect(ui->shadersComboBox, cic, [=](int) {
        update_w_stones();
        update_b_stones();
    });
    ...
    update_w_stones();
    update_b_stones();
    connect(ui->stoneView, &SizeGraphicsView::resized, [=]() {
        update_board_image();
        update_w_stones();
        update_b_stones();
    });
    ...
    connect(ui->dbPathsTreeView->selectionModel(), &QItemSelectionModel::selectionChanged, [this](const QItemSelection &, const QItemSelection &) {
        update_db_selection();
    });
...
void PreferencesDialog::slot_dbsubdir(bool)
{
    ...
    QString dirname = QFileDialog::getExistingDirectory(this, QObject::tr("Add a set of database directories - choose the parent directory"), dir);
    if (dirname.isEmpty())
        return;
    QDir parent(dirname);
    int  skipped = 0;
...
void PreferencesDialog::slot_dbdir(bool)
{
    ...
    for (auto &it : m_dbpath_model.entries())
    {
        ...
        {
            QMessageBox::warning(
                this, tr("Directory already in the list"), tr("The directory could not be added because it already exists in the list."));
            return;
        }
    }
...
void PreferencesDialog::slot_dbcreate(bool)
{
    ...
    QProgressDialog dlg(tr("Creating database from SGF files..."), tr("Abort operation"), 0, 100, this);
...
void PreferencesDialog::slot_autosavedir(bool)
{
    ...
    QString dirname = QFileDialog::getExistingDirectory(this, QObject::tr("Choose a directory for autosaves"), curr);
    if (dirname.isEmpty())
        return;
    QDir d(dirname);
    if (!d.exists())
    {
        QMessageBox::warning(this, tr("Selected directory does not exist"), tr("The selected directory does not exist."));
        return;
    }
...
void PreferencesDialog::update_stone_params()
{
    ...
    if (shader_idx == 0)
    {
        auto vals = std::make_tuple(
            std::make_tuple(ui->blackRoundSlider->value(), ui->blackSpecSlider->value(), ui->blackFlatSlider->value(), ui->blackHardSlider->value()),
            std::make_tuple(ui->whiteRoundSlider->value(), ui->whiteSpecSlider->value(), ui->whiteFlatSlider->value(), ui->whiteHardSlider->value()),
            ui->ambientSlider->value(),
            ui->stripesCheckBox->isChecked());
...
bool PreferencesDialog::verify()
{
    ...
    {
        QMessageBox::warning(this, tr("Invalid slide width"), tr("Please enter valid dimensions for slide export (100x100 or larger)."));
        return false;
    }
    int slide_y = ui->slideYEdit->text().toInt(&ok);
    if (!ok || slide_y < 100 || slide_y > 10000)
    {
        QMessageBox::warning(this, tr("Invalid slide height"), tr("Please enter valid dimensions for slide export (100x100 or larger)."));
        return false;
    }
    if (slide_y > slide_x)
    {
    ...
    if (m_dbpaths_changed)
    {
        ...
        {
            QMessageBox mb(QMessageBox::Question,
                           tr("Missing database files"),
                           tr("<p>No database files could be found in some of the directories configured for the database.</p><p>Do you want to go "
                              "back and create them now?</p>"),
                           QMessageBox::Yes | QMessageBox::No);
...
bool PreferencesDialog::avoid_losing_data()
{
    if (m_changing_host)
    {
        if (!ui->LineEdit_host->text().isEmpty() || !ui->LineEdit_port->text().isEmpty() || !ui->LineEdit_login->text().isEmpty())
        {
            QMessageBox mb(QMessageBox::Question,
                           tr("Unsaved data"),
...
void PreferencesDialog::slot_add_server()
{
    ...
    {
        QMessageBox::warning(this, tr("Title required"), tr("You need to add a title for the account that you want to add."));
        return;
    }
    if (ui->LineEdit_host->text().isEmpty())
    {
        QMessageBox::warning(this, tr("Host required"), tr("You need to add a host name for the account that you want to add."));
        return;
    }
    if (ui->LineEdit_login->text().isEmpty())
    {
        QMessageBox::warning(this, tr("Login name required"), tr("You need to add the login name for the account that you want to add."));
        return;
    }
    ...
    {
        tmp = 0;
        QMessageBox::warning(this, tr("Port required"), tr("You need to add the server's port number for the account that you want to add."));
        return;
    }

    Host new_h(ui->LineEdit_title->text(),
