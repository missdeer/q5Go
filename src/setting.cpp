...
void Setting::extract_lists()
{
    ...
    std::sort(m_hosts.begin(), m_hosts.end(), [](const Host &a, const Host &b) { return a.title < b.title; });
    ...
    std::sort(m_engines.begin(), m_engines.end(), [](const Engine &a, const Engine &b) { return a.title < b.title; });
...
QString Setting::getTranslationsDirectory()
{
    ...
    list << (QString)CFStringGetCStringPtr(bundlePath, CFStringGetSystemEncoding()) + "/Contents/Resources/translations";
#else
    list << DATADIR "/translations" << QApplication::applicationDirPath() + "/translations"
