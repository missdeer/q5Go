...
void ClientWindow::initStatusBar(QWidget * /*parent*/)
{
    ...
    */
    ...
    statusUsers->setWhatsThis(tr("Displays the number of current online players\nand the number of online players you are watching.\nA player you "
                                 "are watching has an entry in the 'watch player:' field."));
    ...
    statusGames->setWhatsThis(
        tr("Displays the number of games currently played on this server and the number of games you are observing or playing"));
    ...
    statusChannel->setWhatsThis(tr(
        "Displays the current channels you are in and the number of users inthere.\nThe tooltip text contains the channels' title and users' names"));
...
void ClientWindow::slot_cbquiet(bool)
{
    ...
    for (auto &h : setting->m_hosts)
    {
        if (h.host == m_active_host.host && h.login_name == m_active_host.login_name && h.title == m_active_host.title)
        {
            h.quiet = val;
            break;
        }
...
void ClientWindow::slot_mouse_players(const Player &p)
{
    ...
    {
        puw = new QMenu(0, 0);
        puw->addAction(tr("match"), [=]() { handle_matchrequest(menu_player_name() + " " + m_menu_player.rank, true, false); });
        puw->addAction(tr("talk"), [=]() { slot_talk(menu_player_name(), QString(), true, false); });
        // puw->insertSeparator();
        puw->addAction(tr("stats"), [=]() { slot_talk(menu_player_name(), QString(), true, false); });
        puw->addAction(tr("stored games"), [=]() { sendcommand("stored " + menu_player_name(), false); });
        puw->addAction(tr("results"), [=]() { sendcommand("result " + menu_player_name(), false); });
        ...
        puw->addAction(tr("trail"), [=]() { sendcommand("trail " + menu_player_name(), false); });
        // puw->insertSeparator();
