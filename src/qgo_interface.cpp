...
class MainWindow_IGS : public MainWindow
{
    ...
    struct preview : public board_preview
    {
        qGoBoard *connector;
        bool      active = true;
        preview(go_game_ptr g, game_state *st, qGoBoard *b) : board_preview(g, st), connector(b) {}
    };
...
public:
    MainWindow_IGS(
        QWidget *parent, std::shared_ptr<game_record> gr, QString screen_key, qGoBoard *connector, bool playing_w, bool playing_b, GameMode mode);
    ~MainWindow_IGS();
...
MainWindow_IGS::MainWindow_IGS(
    QWidget *parent, std::shared_ptr<game_record> gr, QString screen_key, qGoBoard *brd, bool playing_w, bool playing_b, GameMode mode)
    : MainWindow(parent, gr, screen_key, mode)
    , m_connector(brd)
{
    ...
    connect(passButton, &QPushButton::clicked, [this](bool) {
        if (m_connector)
            m_connector->doPass();
    });
    disconnect(undoButton, &QPushButton::clicked, nullptr, nullptr);
    connect(undoButton, &QPushButton::clicked, [this](bool) {
        if (m_connector)
            m_connector->doUndo();
    });
    disconnect(adjournButton, &QPushButton::clicked, nullptr, nullptr);
    connect(adjournButton, &QPushButton::clicked, [this](bool) {
        if (m_connector)
            m_connector->doAdjourn();
    });
    disconnect(resignButton, &QPushButton::clicked, nullptr, nullptr);
    connect(resignButton, &QPushButton::clicked, [this](bool) {
        if (m_connector)
            m_connector->doResign();
    });
    disconnect(doneButton, &QPushButton::clicked, nullptr, nullptr);
    connect(doneButton, &QPushButton::clicked, [this](bool) {
        if (m_connector)
            m_connector->doDone();
    });
...
{
    delete m_cursor;
    m_cursor = m_preview_scene->addRect(0, 0, m_preview_w, m_preview_h, Qt::NoPen, QBrush(Qt::blue));
...
void MainWindow_IGS::board_menu(QGraphicsSceneContextMenuEvent *e, go_game_ptr gr, qGoBoard *, game_state *)
{
    QMenu menu;
...
void qGoIF::game_end(qGoBoard *qb, const QString &txt)
{
    ...
    if (txt.contains("White forfeits"))
        ...
        rs = (((txt.contains(myName) && qb->get_myColorIsBlack()) || (!txt.contains(myName) && !qb->get_myColorIsBlack())) ? "W+T" : "B+T");

    else if (txt.contains("W ", Qt::CaseSensitive) && txt.contains("B ", Qt::CaseSensitive))
    {
        ...
        rs = (((txt.contains(myName) && qb->get_myColorIsBlack()) || (!txt.contains(myName) && !qb->get_myColorIsBlack())) ? "W+R" : "B+R");
...
{
    if (other.m_game->info().name_b != m_game->info().name_b || other.m_game->info().name_w != m_game->info().name_w ||
...
void qGoIF::slot_freegame(bool freegame)
{
    ...
    for (auto qb : boardlist)
        if (qb->get_havegd() && (qb->get_wplayer() == myName || qb->get_bplayer() == myName) && qb->get_mv_counter() < 3)
        {
    ...
    else if (ExtendedTeachingGame && IamTeacher)
    {
        ...
        if (msg.indexOf("#OC:OFF") != -1)
        {
            ...
            return;
                 ...
                 ((myColorIsBlack && (mv_counter % 2)) || (!myColorIsBlack && ((mv_counter % 2 + 1) || mv_counter == -1)) || !haveControls))
        {
...
void qGoBoard::check_requests()
{
    ...
    if (req_free != noREQ && m_game->boardsize() == 19 && mv_counter < 1)
    {
        ...
        if ((m_freegame == FREE && req_free == FREE) || (m_freegame != FREE && req_free == RATED) || req_free == TEACHING)
            return;
