...
protected:
    ...
    void stop_analyzer();
    void pause_eval_updates(bool on)
    {
        m_pause_updates = on;
    }
    bool         pause_analyzer();
...
public:
    ...
    virtual void gtp_played_move(GTP_Process *, int, int) override
    { /* Should not happen.  */
    }
    virtual void gtp_played_resign(GTP_Process *) override
    { /* Should not happen.  */
    }
    virtual void gtp_played_pass(GTP_Process *) override
    { /* Should not happen.  */
    }
    virtual void gtp_setup_success(GTP_Process *) override
    { /* Should not happen.  */
    }
    virtual void gtp_report_score(GTP_Process *, const QString &) override
    { /* Should not happen.  */
    }
...
public:
    GTP_Process(QWidget *parent, GTP_Controller *c, const Engine &engine, int size_x, int size_y, float komi, bool show_dialog = true);
    ~GTP_Process();
    bool started()
    {
        return m_started;
