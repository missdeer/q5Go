...
BoardView::BoardView(QWidget *parent, bool no_sync, bool no_marks)
    ...
    , m_vgrid_outline(1)
    , m_hgrid_outline(1)
    , m_dims(0, 0, DEFAULT_BOARD_SIZE, DEFAULT_BOARD_SIZE)
    , m_hoshis(1)
    , m_never_sync(no_sync)
    , m_no_marks(no_marks)
{
...
Board::Board(QWidget *parent) : BoardView(parent), GTP_Eval_Controller(parent)
{
...
void BoardView::calculateSize()
{
    ...
    m_board_rect = QRect((w - board_pixel_size_x) / 2, (h - board_pixel_size_y) / 2, board_pixel_size_x, board_pixel_size_y);
}
...
QImage BoardView::background_image()
{
    ...
    if (m_show_coords && m_displayed != nullptr)
    {
        ...
        for (int tx = m_crop.x1; tx <= m_crop.x2; tx++)
        {
            ...
            brect.moveCenter(QPoint(m_board_rect.x() + square_size * (hdups + tx), m_wood_rect.y() + center));
            painter.drawText(brect, Qt::AlignCenter, nm);
            brect.moveCenter(QPoint(m_board_rect.x() + square_size * (hdups + tx), m_wood_rect.bottom() - center));
            painter.drawText(brect, Qt::AlignCenter, nm);
        }
        for (int ty = m_crop.y1; ty <= m_crop.y2; ty++)
        {
            ...
            QRect   brect = fm2.boundingRect(nm);
            brect         = fm2.boundingRect(brect, Qt::AlignCenter, nm);
            brect.moveCenter(QPoint(m_wood_rect.x() + center, m_board_rect.y() + square_size * (vdups + ty)));
            painter.drawText(brect, Qt::AlignCenter, nm);
            brect.moveCenter(QPoint(m_wood_rect.right() - center, m_board_rect.y() + square_size * (vdups + ty)));
            painter.drawText(brect, Qt::AlignCenter, nm);
        }
    }
...
/* Render a mark in SVG at center position CX/CY in a square with a side length of FACTOR.
   ...
   little bit.  */
static bool add_mark_svg(svg_builder &      svg,
                         double             cx,
                         double             cy,
                         double             factor,
                         mark               m,
                         ...
                         QString            color_override = QString())
{
    ...
    {
        svg.square_at(cx, cy, factor * 0.9, "white", "none");
    }
    if (count_val != 0)
    {
        int len = max_number > 99 ? 3 : max_number > 9 ? 2 : 1;
        svg.text_at(cx, cy, factor, len, QString::number(count_val), mark_col, fi);
        return true;
    }
    ...
    {
    case mark::circle:
        svg.circle_at(cx, cy, factor * 0.25, "none", mark_col);
        break;
    case mark::square:
        svg.square_at(cx, cy, factor * 0.8 / M_SQRT2, "none", mark_col);
        break;
    case mark::triangle:
        svg.triangle_at(cx, cy, factor * 0.8, "none", mark_col);
        break;
    case mark::terr:
        ...
        /* fall through */
    case mark::cross:
        svg.cross_at(cx, cy, factor * 0.8, mark_col);
        break;
    case mark::num:
        svg.text_at(cx, cy, factor, 0, QString::number(me), mark_col, fi);
        break;
    case mark::letter:
        svg.text_at(cx, cy, factor, 0, convert_letter_mark(me), mark_col, fi);
        break;
    case mark::text:
        svg.text_at(cx, cy, factor, 0, QString::fromStdString(mstr), mark_col, fi);
        break;
    case mark::dead:
        return false;
...
QByteArray BoardView::render_svg(bool do_number, bool coords)
{
    ...
    if (coords)
    {
        ...
        for (int y = 0; y < rows; y++)
        {
            ...
            if (m_sel.aligned_left(m_dims))
                svg.fixed_height_text_at(dist, center_y, factor * coord_factor, QString::number(m_dims.height() - ry), "black", fi, true);
            if (m_sel.aligned_right(m_dims))
                svg.fixed_height_text_at(w - dist, center_y, factor * coord_factor, QString::number(m_dims.height() - ry), "black", fi, true);
        }
        for (int x = 0; x < cols; x++)
        {
            ...
            if (rx < 9)
                ...
                label = QChar('A' + rx + 1);
            if (m_sel.aligned_top(m_dims))
                svg.fixed_height_text_at(center_x, dist, factor * coord_factor, label, "black", fi, true);
            if (m_sel.aligned_bottom(m_dims))
                svg.fixed_height_text_at(center_x, h - dist, factor * coord_factor, label, "black", fi, true);
        }
    }
    ...
    {
        QString width = "1";
        if ((x == 0 && m_sel.aligned_left(m_dims)) || (x + 1 == cols && m_sel.aligned_right(m_dims)))
            width = "2";
        svg.line(offset_x + x * factor, offset_y + top, offset_x + x * factor, offset_y + (rows - 1) * factor + bot, "black", width);
    }
    for (int y = 0; y < rows; y++)
    {
        QString width = "1";
        if ((y == 0 && m_sel.aligned_top(m_dims)) || (y + 1 == rows && m_sel.aligned_bottom(m_dims)))
            width = "2";
        svg.line(offset_x + lef, offset_y + y * factor, offset_x + (cols - 1) * factor + rig, offset_y + y * factor, "black", width);
    }
    for (int y = 0; y < rows; y++)
    {
        for (int x = 0; x < cols; x++)
        {
            ...
            if (c != none)
            {
                svg.circle_at(center_x, center_y, factor * 0.45, c == black ? "black" : "white", c == black ? "none" : "black");
            }
            add_mark_svg(svg,
                         center_x,
                         center_y,
                         ...
                         m,
                         extra,
                         m == mark::text ? b.mark_text_at(rx, ry) : "",
                         mark::none,
                         0,
                         c,
                         v,
...
void BoardView::draw_grid(QPainter &painter, bit_array hgrid, bit_array vgrid, const bit_array &hidden)
{
    ...
    auto draw_vertical = [&](int tx, bool center = false) {
        ...
        for (int ty = begin_y * 2; ty < end_y * 2; ty++)
        {
            ...
            {
                painter.drawLine(line_offx + tx * square_size,
                                 line_offy + first * square_size / 2,
                                 line_offx + tx * square_size,
                                 line_offy + last * square_size / 2);
                first = -2;
            }
        }
        if (first != -2)
            painter.drawLine(
                line_offx + tx * square_size, line_offy + first * square_size / 2, line_offx + tx * square_size, line_offy + last * square_size / 2);
    };
    auto draw_horizontal = [&](int ty, bool center = false) {
        ...
        for (int tx = begin_x * 2; tx < end_x * 2; tx++)
        {
            ...
            {
                painter.drawLine(line_offx + first * square_size / 2,
                                 line_offy + ty * square_size,
                                 line_offx + last * square_size / 2,
                                 line_offy + ty * square_size);
                first = -2;
            }
        }
        if (first != -2)
            painter.drawLine(
                line_offx + first * square_size / 2, line_offy + ty * square_size, line_offx + last * square_size / 2, line_offy + ty * square_size);
    };
    ...
    for (int tx = 0; tx < visx; tx++)
        for (int ty = 0; ty < visy; ty++)
        {
            ...
            {
                painter.drawEllipse(QPoint(line_offx + (tx + dups_x) * square_size, line_offy + (ty + dups_y) * square_size), hoshi_size, hoshi_size);
            }
        }
}
...
std::pair<stone_color, stone_type> BoardView::stone_to_display(
    const go_board &b, const bit_array *visible, stone_color to_move, int x, int y, const go_board &vars, int var_type)
{
...
Board::ram_result Board::render_analysis_marks(
    svg_builder &svg, double svg_factor, double cx, double cy, const QFontInfo &fi, int x, int y, bool child_mark, int v, int max_number)
{
    if (!have_analysis())
    ...
    else
    {
        ...
        svg.text_at(cx, cy, svg_factor, 4, QString::number(shown_val * 100, 'f', 1), "black", fi);
    }
...
QPixmap BoardView::draw_position(int default_vars_type)
{
    ...
    if (setting->values.time_warn_board && m_time != 0 && m_displayed->was_move_p())
    {
        if (m_time < 100)
        {
            mn_board.set_mark(m_displayed->get_move_x(), m_displayed->get_move_y(), mark::num, m_time);
    ...
    if (m_cont != nullptr)
    {
        ...
        for (int tx = 0; tx < visx + 2 * dups_x; tx++)
            for (int ty = 0; ty < visy + 2 * dups_y; ty++)
            {
                ...
                QString     text_col      = (entry_w.count > 0 && entry_b.count > 0 ? "#555555" : entry_w.count > 0 ? "white" : "black");
                auto        stone_display = stone_to_display(mn_board, visible, to_move, x, y, vars, var_type);
                stone_color sc            = stone_display.first;
                ...
                if (m_cont_view == pattern_cont_view::letters)
                {
                    ...
                    add_mark_svg(svg, cx, cy, svg_factor, mark::letter, val, "", mark::none, 0, sc, 0, 0, false, false, fi, text_col);
                }
                else if (m_cont_view == pattern_cont_view::numbers)
                {
                    std::string t = QString::number(entry.count).toStdString();
                    add_mark_svg(svg, cx, cy, svg_factor, mark::text, 0, t, mark::none, 0, sc, 0, 0, false, false, fi, text_col);
                }
                else
                {
    ...
    if (m_visualize_connections)
    {
        ...
        for (int tx = 0; tx + 1 < visx + 2 * dups_x; tx++)
            for (int ty = 0; ty < visy + 2 * dups_y; ty++)
            {
                ...
                {
                    painter.drawLine(
                        line_offx + tx * square_size, line_offy + ty * square_size, line_offx + (tx + 1) * square_size, line_offy + ty * square_size);
                }
            }
        for (int tx = 0; tx < visx + 2 * dups_x; tx++)
            for (int ty = 0; ty + 1 < visy + 2 * dups_y; ty++)
            {
                ...
                {
                    painter.drawLine(
                        line_offx + tx * square_size, line_offy + ty * square_size, line_offx + tx * square_size, line_offy + (ty + 1) * square_size);
                }
            }
    }
    ...
    for (int tx = 0; tx < visx + 2 * dups_x; tx++)
        for (int ty = 0; ty < visy + 2 * dups_y; ty++)
        {
            ...
            {
                painter.drawPixmap(shadow_offx + tx * square_size, shadow_offy + ty * square_size, imageHandler->getStonePixmaps()->last());
            }
        }
    ...
    for (int tx = 0; tx < visx + 2 * dups_x; tx++)
        for (int ty = 0; ty < visy + 2 * dups_y; ty++)
        {
            ...
            {
                int bp = b.bitpos(x, y);
                painter.drawPixmap(stone_offx + tx * square_size, stone_offy + ty * square_size, choose_stone_pixmap(sc, type, bp));
            }
        }
    ...
    if (!m_no_marks)
    {
        ...
        transform.translate(m_board_rect.x() - m_wood_rect.x() - square_size / 2, m_board_rect.y() - m_wood_rect.y() - square_size / 2);
...
void BoardView::updateCovers()
{
    ...
    coverRight->setRect(right_edge, sides_top_edge, sceneRect.right() - right_edge, sides_bot_edge - sides_top_edge);
...
{
    return go_pattern(m_displayed->get_board(), m_sel.x2 + 1 - m_sel.x1, m_sel.y2 + 1 - m_sel.y1, m_sel.x1, m_sel.y1);
}
...
void SimpleBoard::mousePressEvent(QMouseEvent *e)
{
    ...
    if (e->button() == Qt::LeftButton)
    {
        ...
        if (col == none)
            col = (e->modifiers() == Qt::ShiftModifier ? black : e->modifiers() == Qt::ControlModifier ? white : st->to_move());
...
void Board::mousePressEvent(QMouseEvent *e)
{
    ...
    if (m_eval_state != nullptr && ((e->modifiers() == Qt::ShiftModifier && e->button() == Qt::LeftButton) || e->button() == Qt::MiddleButton))
    {
        ...
        if (evchild != nullptr)
        {
            ...
            double      wwr     = 1 - bwr;
            comment             = (comment.arg(QString::number(100 * wwr, 'f', 1))
                           .arg('%')
                           .arg(QString::number(100 * bwr, 'f', 1))
                           .arg('%')
    ...
    if (e->modifiers() == Qt::ControlModifier && (m_game_mode == modeNormal || m_game_mode == modeObserve))
    {
...
void Board::eval_received(const analyzer_id &id, const QString &move, int visits, bool have_score)
{
    ...
    m_board_win->set_eval(move, m_primary_eval, m_primary_have_score, m_primary_score, m_displayed->to_move(), visits);
    sync_appearance();
}
...
FigureView::FigureView(QWidget *parent, bool no_sync, bool no_marks) : BoardView(parent, no_sync, no_marks)
{
    m_figure_view = true;
}
...
void FigureView::contextMenuEvent(QContextMenuEvent *e)
{
    ...
    if (m_displayed->visible_inherited() != nullptr)
    {
        ...
        if (m_displayed->visible() != nullptr)
            menu.addAction(QObject::tr("C&lear visibility state, inherit from parent"), [this](bool) {
                m_displayed->set_visible(nullptr);
                sync_appearance(true);
            });
    }
