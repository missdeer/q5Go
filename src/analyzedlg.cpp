...
AnalyzeDialog::AnalyzeDialog(QWidget *parent, const QString &filename) : QMainWindow(parent), GTP_Eval_Controller(parent)
{
    setupUi(this);
    ...
    connect(jobView->selectionModel(), &QItemSelectionModel::selectionChanged, [=](const QItemSelection &, const QItemSelection &) {
        update_progress();
    });
...
AnalyzeDialog::job::job(AnalyzeDialog *dlg, QString &title, go_game_ptr gr, int n_seconds, int n_lines, engine_komi k, bool comments)
    : m_dlg(dlg)
    , m_title(title)
    , m_game(gr)
...
void AnalyzeDialog::job::show_window(bool done)
{
    if (m_win == nullptr)
    {
        ...
        m_connection = connect(m_win, &MainWindow::signal_closeevent, [this]() {
            m_win = nullptr;
...
void AnalyzeDialog::discard_job(bool done)
{
    ...
    if (j == nullptr)
        return;
    QMessageBox::StandardButton choice;
    choice = QMessageBox::warning(this, PACKAGE, tr("Really discard selected job?"), QMessageBox::Yes | QMessageBox::Cancel, QMessageBox::Yes);
        ...
        if (e.visits > 0)
        {
            ...
            if (next && next->was_move_p())
            {
                ...
                if (cname != nextcname)
                {
                    ...
                    {
                        if (variations[cnt]->get_move_x() == next->get_move_x() && variations[cnt]->get_move_y() == next->get_move_y())
                        {
...
void AnalyzeDialog::start_job()
{
    ...
    {
        QMessageBox::warning(this, PACKAGE, tr("File has a different boardsize than selected!"));
        return;
    }
    ...
    m_all_jobs.emplace_front(this, f, gr, secondsEdit->text().toInt(), maxlinesEdit->text().toInt(), k, commentsCheckBox->isChecked());
