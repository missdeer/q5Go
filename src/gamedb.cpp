...
void GameDB_Data_Controller::slot_load_complete()
{
    ...
    if (db_data->errors_kombilo)
        s += tr("<p>These errors could also mean that there is a 32/64-bit mismatch between this program and the version Kombilo that generated the "
                "database.</p>");
    if (!s.isEmpty())
    {
...
template<class T> uint32_t read_uint(QDataStream &ds)
{
    T    v;
    char v_raw[sizeof(T)];
...
bool GameDB_Data::read_q5go_db(const QDir &dbdir, int base_id, unsigned max_mb, bool read_movelist)
{
    ...
    try
    {
        ...
        for (uint32_t i = 0; i < n_games; i++)
        {
            ...
            total += strid_sz * 7 + 4;
            m_all_entries.emplace_back(base_id + i,
                                       dbdir.absolutePath(),
                                       filename,
                                       strtable[nmw_id],
                                       strtable[rkw_id],
...
#ifdef CHECKING
            if (elt.finalpos_w.raw_n_elts() != elt.finalpos_b.raw_n_elts() || elt.finalpos_w.raw_n_elts() != elt.finalpos_c.raw_n_elts())
                abort();
#endif
            total += 3 * sz;
...
bool GameDB_Data::read_extra_file(QDataStream &ds, int base_id, int boardsize, unsigned max_mb, bool cache_movelist)
{
    ...
    beg      = std::lower_bound(beg, end, base_id, [](const gamedb_entry &e, int val) { return e.id < val; });
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        auto pos = std::lower_bound(beg, end, gameid, [](const gamedb_entry &e, int val) { return e.id < val; });
        if (ds.readRawData(i_c, sizeof i_c) != sizeof i_c)
            throw db_errors_found();
...
gamedb_model::gamedb_model(bool ps) : m_patternsearch(ps)
{
...
{
    std::sort(std::begin(m_entries), std::end(m_entries), [](unsigned a, unsigned b) {
...
void gamedb_model::apply_filter(const QString &p1, const QString &p2, const QString &event, const QString &dtfrom, const QString &dtto)
{
    beginResetModel();
