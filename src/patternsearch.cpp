...
PatternSearchWindow::PatternSearchWindow() : ui(new Ui::PatternSearchWindow), m_model(true), m_previewer(new FigureView(nullptr, true, true))
{
    ui->setupUi(this);
    go_board  b(19);
    game_info info;
    ...
    connect(ui->dbListView->selectionModel(), &QItemSelectionModel::selectionChanged, [this](const QItemSelection &, const QItemSelection &) {
        update_selection();
    });
...
{
    QMenu menu;
    menu.addAction(QIcon(":/images/trash.png"), QObject::tr("&Forget this search"), [this, &p]() {
        delete p.pixmap;
        clear_preview_cursor();
        m_previews.erase(std::remove_if(std::begin(m_previews), std::end(m_previews), [&p](const preview &other) { return &p == &other; }),
                         std::end(m_previews));
        m_search_cont = nullptr;
        m_stats_scene->clear();
        m_result_scene->clear();
...
void PatternSearchWindow::slot_choose_view(bool)
{
    pattern_cont_view v = (ui->viewPercent->isChecked() ? pattern_cont_view::percent
                                                        : ui->viewLetters->isChecked() ? pattern_cont_view::letters : pattern_cont_view::numbers);
    ui->boardView->set_cont_view(v);
    setting->writeIntEntry("SEARCH_VIEW", v == pattern_cont_view::letters ? 0 : v == pattern_cont_view::numbers ? 1 : 2);
}
...
void PatternSearchWindow::redraw_stats()
{
    ...
    std::sort(rank.begin(), rank.end(), [this](int a, int b) { return (*m_search_cont)[a].get(none).count > (*m_search_cont)[b].get(none).count; });
...
search_runnable::search_runnable(
    PatternSearchWindow *parent, gamedb_model *m, gamedb_model::search_result **r, go_pattern &&p, std::atomic<long> *cur, std::atomic<long> *max)
    : m_model(m)
    , m_result(r)
    , m_cur(cur)
    , m_max(max)
...
void PatternSearchWindow::slot_completed()
{
    ...
    for (size_t i = 0; i < b.bitsize(); i++)
        rank[i] = i;
    std::sort(rank.begin(), rank.end(), [&p](int a, int b) { return p.cont[a].get(none).count > p.cont[b].get(none).count; });
    for (size_t i = 0; i < b.bitsize(); i++)
...
void PatternSearchWindow::pattern_search(bool all_games)
{
    ...
    if (!ui->anchorAction->isChecked())
        p.clear_alignment();
    search_runnable *r = new search_runnable(this, &m_model, &m_result, std::move(p), &m_progress, &m_progress_max);
...
void PatternSearchWindow::slot_using()
{
    ...
    txt += tr("<p>Select a rectangle using the right mouse button. Search in the current game list by pressing S, or in all the games by pressing A "
              "(or use the menus and icons).</p>");
