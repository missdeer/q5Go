...
InfoType Parser::put_line(const QString &txt)
{
    ...
    else if (!ok || (memory_str.contains("File") && !line.contains("File")))
    {
        ...
        if (line.indexOf("#>") != -1 && !memory_str.isNull() && !memory_str.contains("File"))
            return NOCLIENTMODE;
...
InfoType Parser::cmd9(const QString &line)
{
    ...
    else if (line.contains("has restarted your game") || line.contains("has restored your old game"))
    {
    ...
    // 9 frosla withdraws the match offer.
    // 9 guest17 declines your request for a match.
    else if (line.contains("declines your request for a match") || line.contains("withdraws the match offer"))
    {
    ...
    else if ((line.contains("has resigned the game")) || (line.contains("has run out of time")))
    {
...
InfoType Parser::cmd21(const QString &line)
{
    ...
    else if (line.contains("{Game"))
    {
        ...
        if (line.contains("resigns.") || line.contains("adjourned.") || line.contains(" : W ") || line.contains(" : B ") ||
            line.contains("forfeits on") || line.contains("lost by"))
        {
            // remove game from list
            aGame->nr = line.section(' ', 1, 1).remove(':');
...
InfoType Parser::cmd42(const QString &txt)
{
    ...
    if (aPlayer->nmatch)
    {
        ...
        if (!nmatchString.isEmpty())
        {
            ...
            if (s1 == "1")
                aPlayer->nmatch_settings = "Jap. BY : " + t1min + " min. + " + t2min + " secs./st. ";
            else
                aPlayer->nmatch_settings = "Can. BY : " + t1min + " min. + " + t3min + " min. /" + s1 + " st. ";
...
InfoType Parser::cmd63(const QString &line)
{
    ...
    {
        QString player    = line.section(' ', 1, 1);
        QString condition = line.section(' ', 7, 7) + "x" + line.section(' ', 7, 7) + " - " +
                            QString::number(int(line.section(' ', 2, 2).toInt() / 60)) + " +" +
                            QString::number(int(line.section(' ', 3, 3).toInt() / 60)).rightJustified(3) + "' (" + line.section(' ', 4, 4) + ") H -" +
