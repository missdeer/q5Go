...
template<class T> class pref_vec_model : public QAbstractItemModel
{
...
public:
    pref_vec_model(std::vector<T> init) : m_entries(init) {}
    pref_vec_model(std::vector<T> init, const std::vector<T> &extra) : m_entries(init), m_extra(extra) {}
