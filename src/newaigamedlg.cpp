...
template<class UI> AIGameDlg<UI>::AIGameDlg(QWidget *parent) : QDialog(parent), ui(new UI)
{
    ui->setupUi(this);
    setModal(true);
    ...
    connect(ui->boardsizeSpinBox, changed, [this](int v) {
...
template<class UI> AIGameDlg<UI>::~AIGameDlg()
{
    delete ui;
}

template<class UI> QString AIGameDlg<UI>::game_to_load()
{
    return ui->gameToLoad->text();
}

template<class UI> std::pair<int, int> AIGameDlg<UI>::board_size()
{
    return std::make_pair(ui->boardsizeSpinBox->value(), ui->boardsizeYSpinBox->value());
}

template<class UI> int AIGameDlg<UI>::handicap()
{
...
template<class UI> time_settings AIGameDlg<UI>::timing()
{
    time_settings t;
...
template<class UI> go_game_ptr AIGameDlg<UI>::create_game_record()
{
    int                          hc   = handicap();
