...
protected:
    ...
    bool have_selection();
    /* Overridden by Board.  */
    virtual stone_color cursor_color(int, int, stone_color)
    {
        return none;
    }
    virtual int extract_analysis(go_board &)
    {
        return 0;
    }
    enum class ram_result
    {
        none,
        hide,
        nohide
    };
    virtual ram_result render_analysis_marks(svg_builder &, double, double, double, const QFontInfo &, int, int, bool, int, int)
    {
        return ram_result::none;
    }
    virtual bool have_analysis()
    {
        return false;
    }
    std::pair<stone_color, stone_type> stone_to_display(
        const go_board &, const bit_array *visible, stone_color to_move, int x, int y, const go_board &vars, int var_type);

    void update_rect_select(int, int);
    void end_rect_select()
    {
        m_rect_down_x = m_rect_down_y = -1;
    }
...
public:
    ...
    void set_board_win(MainWindow *w)
    {
        m_board_win = w;
    }
    ...
    game_state * displayed()
    {
        return m_displayed;
    }

    void set_cont_data(const std::vector<pattern_cont_data> *d)
    {
        m_cont = d;
        sync_appearance();
    }
    void clear_cont_data()
    {
        m_cont = nullptr;
        sync_appearance();
    }
    void set_cont_view(pattern_cont_view v)
    {
        m_cont_view = v;
        if (m_cont != nullptr)
            sync_appearance();
    ...
    stone_color to_move()
    {
        return m_displayed->to_move();
    }

    void set_never_sync(bool n)
    {
        m_never_sync = n;
    }
    void    set_margin(int);
    ...
    void set_drawn_selection(board_rect r)
    {
        m_drawn_sel = std::move(r);
        sync_appearance();
    }
    board_rect get_selection() const
    {
        return m_sel;
    }
    void set_selection(board_rect r)
    {
        m_sel = std::move(r);
        updateCovers();
    }
...
public:
    ...
    void         set_stone_color(stone_color c)
    {
        m_forced_color = c;
    }
...
public:
    ...
    void setMarkType(mark t)
    {
        m_edit_mark = t;
    }

    void navIntersection();
    void set_rect_select(int on)
    {
        m_request_mark_rect = on;
    }
    ...
    void set_player_colors(bool w, bool b)
    {
        m_player_is_w = w;
        m_player_is_b = b;
    }
    bool player_is(stone_color c)
    {
        return c == black ? m_player_is_b : m_player_is_w;
    }
...
protected:
    ...
    virtual ram_result  render_analysis_marks(
         svg_builder &, double svg_factor, double cx, double cy, const QFontInfo &, int x, int y, bool child_mark, int v, int max_number) override;
