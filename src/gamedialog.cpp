...
GameDialog::GameDialog(QWidget *parent, GSName gs, const QString &name, const QString &rk, const QString &opponent, const QString &opp_rk)
    : QDialog(parent)
    , gsname(gs)
    , myName(name)
    , myRk(rk)
...
void GameDialog::slot_opponentopen(const QString &opp)
{
    ...
    QString send = "tell " + opponent + " CLIENT: <" + PACKAGE + " " + VERSION + "> match " + me + " wants handicap " + handicapSpin->text() +
                   ", komi " + komiSpin->text();
...
void GameDialog::slot_offer(bool active)
{
    ...
    if (is_nmatch)
        ...
        client_window->sendcommand("nmatch " + playerOpponentEdit->text() + color + //" W " +
                                       handicapSpin->text() + " " + boardSizeSpin->text() + " " + QString::number(timeSpin->value() * 60) + " " +
                                       QString::number(byoTimeSpin->value() * 60) + " 25 0 0 0",
                                   true); // carefull : 25 stones hard coded : bad
    else
        client_window->sendcommand(
            "match " + playerOpponentEdit->text() + color + boardSizeSpin->text() + " " + timeSpin->text() + " " + byoTimeSpin->text(), false);
