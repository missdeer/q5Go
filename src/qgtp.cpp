...
GTP_Process::GTP_Process(QWidget *parent, GTP_Controller *c, const Engine &engine, int size_x, int size_y, float komi, bool show_dialog)
    : m_name(engine.title)
    , m_dlg(parent, TextView::type::gtp)
    , m_controller(c)
    , m_size_x(size_x)
    , m_size_y(size_y)
    , m_komi(komi)
...
void GTP_Process::startup_part2(const QString &response)
{
    ...
    if (m_size_x == m_size_y)
        ...
        send_request(QString("rectangular_boardsize ") + QString::number(m_size_x) + " " + QString::number(m_size_y),
                     &GTP_Process::startup_part3,
                     &GTP_Process::rect_board_err_receiver);
}
...
{
    QString cmd    = (analyze && m_analyze_kata ? "kata-genmove_analyze" : analyze && m_analyze_lz ? " lz-genmove_analyze" : "genmove");
...
void GTP_Eval_Controller::gtp_eval(GTP_Process *p, const QString &s, bool kata_format)
{
    ...
    for (auto &e : moves)
    {
        ...
        if (count == 0)
        {
            ...
            m_eval_state->set_eval_data(visits, to_move == white ? 1 - wr : wr, scorem, scored, id);
        }
#if 0
            ...
            for (auto &pm : pvmoves)
            {
                ...
                if (i >= 0 && i < szx && j >= 0 && j < szy)
                {
                    ...
                    if (pv_first)
                    {
                        ...
                        next->set_eval_data(visits, to_move == white ? 1 - wr : wr, scorem, scored, id);
