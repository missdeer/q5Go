...
public:
    undo_move_entry(const QString &op, game_state *parent, game_state *move, size_t idx) : undo_entry(op), m_parent(parent), m_move(move), m_idx(idx)
    {
    }
...
public:
    undo_info_entry(const QString &op, go_game_ptr game, game_info oldi, game_info newi)
        : undo_entry(op)
        , m_game(game)
        , m_before(std::move(oldi))
        , m_after(std::move(newi))
    {
    }
    virtual game_state *apply_undo() override;
    virtual game_state *apply_redo() override;
...
public:
    undo_comment_entry(const QString &op, game_state *move, std::string oldc, std::string newc)
        : undo_entry(op)
        , m_move(move)
        , m_before(std::move(oldc))
        , m_after(std::move(newc))
    {
    }
...
public:
    undo_edit_entry(const QString &op, game_state *move, go_board oldb, stone_color oldc, go_board newb, stone_color newc, bool was_edit_mode)
        : undo_entry(op)
        , m_move(move)
        , m_before(std::move(oldb))
        , m_after(std::move(newb))
        , m_before_to_move(oldc)
        , m_after_to_move(newc)
        , m_was_edit_mode(was_edit_mode)
    {
    }
    ...
    , m_svg_dlg(this)
{
    ...
    local_stone_sound = setting->readBoolEntry(mode == modeMatch ? "SOUND_MATCH_BOARD"
                                                                 : mode == modeObserve || mode == modeObserveGTP || mode == modeObserveMulti
                                                                       ? "SOUND_OBSERVE"
                                                                       : mode == modeComputer ? "SOUND_COMPUTER" : "SOUND_NORMAL");
    ...
    connect(leaveMatchButton, &QPushButton::clicked, [this](bool) {
        m_remember_mode = modeNormal;
        if (m_gamemode == modePostMatch)
            setGameMode(modeNormal);
        remove_connector();
    });
    connect(editAppendButton, &QPushButton::clicked, [this](bool) {
        leave_edit_append();
        setGameMode(m_remember_mode);
    });
    connect(editReplaceButton, &QPushButton::clicked, [this](bool) {
        leave_edit_modify();
        setGameMode(m_remember_mode);
    });
    connect(editInsertButton, &QPushButton::clicked, [this](bool) {
        leave_edit_prepend();
        setGameMode(m_remember_mode);
    });
    ...
    connect(normalTools->anPauseButton, &QToolButton::clicked, [=](bool on) { anPause->setChecked(on); });
    ...
    connect(db_data_controller, &GameDB_Data_Controller::signal_prepare_load, [this]() { searchPattern->setEnabled(false); });
    connect(db_data, &GameDB_Data::signal_load_complete, [this]() { searchPattern->setEnabled(true); });
...
void MainWindow::slotEditDelete(bool)
{
    ...
    st->walk_tree([this, parent](game_state *s) -> bool {
        gfx_board->transfer_displayed(s, parent);
        return true;
    });
...
{
    bool is_online = (m_gamemode == modeMatch || m_gamemode == modeObserve || m_gamemode == modeObserveMulti || m_gamemode == modeTeach);
    if (is_online)
    {
...
void MainWindow::remove_nodes(const std::vector<game_state *> &to_delete)
{
    ...
    for (auto st : to_delete)
    {
        st->walk_tree([board = gfx_board, root](game_state *st) {
            board->transfer_displayed(st, root);
            return true;
        });
        st->disconnect();
    }
...
void MainWindow::setMoveData(const game_state *gs)
{
    ...
    bool     good_mode =
        mode == modeNormal || mode == modeObserve || mode == modeObserveMulti || mode == modeObserveGTP || mode == modeBatch || mode == modePostMatch;
...
MainWindow::move_result MainWindow::make_move(game_state *from, int x, int y)
{
    ...
    if (m_gamemode == modeNormal)
    {
        ...
        push_undo(std::make_unique<undo_move_entry>("Play " + qn, from, st_new, from->find_child_idx(st_new)));
    }
...
void MainWindow::doPass()
{
    ...
    if (m_gamemode == modeNormal || m_gamemode == modeComputer)
    {
        ...
        push_undo(std::make_unique<undo_move_entry>("Pass", st, st_new, st->find_child_idx(st_new)));
...
void MainWindow::doCountDone()
{
    ...
    push_undo(std::make_unique<undo_move_entry>("Score the game", m_pos_before_edit, st, m_pos_before_edit->find_child_idx(st)));
    m_pos_before_edit = nullptr;
    doRealScore(false);
}
...
void MainWindow::notice_mark_change(const go_board &newpos)
{
    ...
    game_state *    st = gfx_board->displayed();
    const go_board &b  = st->get_board();
    push_undo(std::make_unique<undo_edit_entry>("Edit marks", st, b, st->to_move(), newpos, st->to_move(), false));
}
...
void MainWindow::leave_edit_modify()
{
    game_state *    st = gfx_board->displayed();
    const go_board &b  = st->get_board();
    push_undo(std::make_unique<undo_edit_entry>(
        "Edit position", m_pos_before_edit, m_pos_before_edit->get_board(), m_pos_before_edit->to_move(), b, st->to_move(), true));
...
void MainWindow::leave_edit_append()
{
    ...
    push_undo(std::make_unique<undo_move_entry>("Append edited position", m_pos_before_edit, new_st, m_pos_before_edit->find_child_idx(new_st)));
...
void MainWindow::leave_edit_prepend()
{
    ...
    push_undo(std::make_unique<undo_insert_entry>("Insert edited position", parent, new_st, parent->find_child_idx(new_st)));
    gfx_board->set_displayed(new_st);
    m_pos_before_edit = nullptr;
    gfx_board->setModified();
    editPosButton->setText(tr("Edit position"));
...
void MainWindow::add_engine_pv(game_state *st, game_state *pv)
{
    ...
    push_undo(std::make_unique<undo_move_entry>("Append engine PV", st, pv, st->find_child_idx(pv)));
...
MainWindow_GTP::MainWindow_GTP(
    QWidget *parent, go_game_ptr gr, QString opener_scrkey, const Engine &program, const time_settings &ts, bool b_is_comp, bool w_is_comp)
    : MainWindow(parent, gr, opener_scrkey, modeComputer, ts)
    , GTP_Eval_Controller(this)
{
    game_state *root = gr->get_root();
    game_state *st   = root;
...
MainWindow_GTP::MainWindow_GTP(QWidget *            parent,
                               go_game_ptr          gr,
                               game_state *         st,
                               QString              opener_scrkey,
                               const Engine &       program,
                               const time_settings &ts,
                               bool                 b_is_comp,
                               bool                 w_is_comp)
    : MainWindow(parent, gr, opener_scrkey, modeComputer, ts)
    , GTP_Eval_Controller(this)
...
MainWindow_GTP::MainWindow_GTP(QWidget *            parent,
                               go_game_ptr          gr,
                               QString              opener_scrkey,
                               const Engine &       program_w,
                               const Engine &       program_b,
                               const time_settings &ts,
                               int                  n_games,
                               bool                 book)
    : MainWindow(parent, gr, opener_scrkey, modeObserveGTP, ts)
    , GTP_Eval_Controller(this)
{
    game_state *root    = gr->get_root();
...
void MainWindow_GTP::time_loss(stone_color col)
{
    ...
    setGameMode(modeNormal);
    QMessageBox mb(QMessageBox::Information, tr("Clock has run out"), report, QMessageBox::Ok | QMessageBox::Default);
    mb.exec();
}
...
void MainWindow_GTP::eval_received(const analyzer_id &id, const QString &move, int visits, bool have_score)
{
    ...
    set_eval(move, m_primary_eval, m_primary_have_score, m_primary_score, st->to_move(), visits);
    update_game_tree();
}
...
void MainWindow::setTimes(int bt, const QString &bstones, int wt, const QString &wstones, bool warn_black, bool warn_white, int timer_cnt)
{
...
void MainWindow::set_eval(const QString &move, double eval, bool have_score, double score, stone_color to_move, int visits)
{
    if (move.isEmpty())
    {
        clear_primary_eval();
...
void MainWindow::set_2nd_eval(const QString &move, double eval, bool have_score, double score, stone_color to_move, int visits)
{
    if (move.isEmpty())
    {
        clear_2nd_eval();
