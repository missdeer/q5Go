...
class go_board
{
    ...
    public:
        stone_unit(const bit_array &stones, int n_liberties)
            : m_stones(stones)
            , m_n_liberties(n_liberties)
            , m_alive(true)
            , m_seki(false)
            , m_any_terr(false)
            , m_real_terr(false)
            , m_seki_neighbour(false)
        {
        }
    };
    /* Used during scoring.  */
...
public:
    ...
    go_board(const go_board &other)
        ...
        , m_stones_b(other.m_stones_b)
        , m_stones_w(other.m_stones_w)
        , m_units_b(other.m_units_b)
        , m_units_w(other.m_units_w)
        , m_units_t(other.m_units_t)
        , m_units_st(other.m_units_st)
        , m_marks(other.m_marks)
        , m_mark_extra(other.m_mark_extra)
        , m_mark_text(other.m_mark_text)
    {
    }
    ...
    go_board(const go_board &other, stone_color)
        ...
        , m_row_bottom(other.m_row_bottom)
        , m_mask(other.m_mask)
        , m_stones_b(other.bitsize())
        , m_stones_w(other.bitsize())
    {
    }
    ...
    bool set_mark(int x, int y, mark m, mextra extra)
    {
        init_marks(false);
        int bp = bitpos(x, y);
        if (m_marks[bp] == m && ((m != mark::num && m != mark::letter) || m_mark_extra[bp] == extra))
            return false;
...
private:
    ...
    void      scoring_flood_fill(bit_array &fill, const bit_array &w_stones, const bit_array &b_stones, bool &neighbours_w, bool &neighbours_b);
