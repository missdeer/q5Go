...
static std::array<tutorial, 7> tutorials = {tutorial {QObject::tr("The Rules of Go"), ":/Tutorials/sgfs/rules.sgf", 12},
...
Tutorial_Slideshow::Tutorial_Slideshow(QWidget *parent) : BaseSlideView<Ui::SlideshowDialog, QWidget>(parent)
{
    ...
    connect(ui->goFirstButton, &QPushButton::clicked, [this](bool) {
        set_active(m_game->get_root());
        update_buttons();
    });
    connect(ui->goLastButton, &QPushButton::clicked, [this](bool) {
    ...
    connect(ui->goNextButton, &QPushButton::clicked, [this](bool) {
        set_active(m_board_exporter->displayed()->next_primary_move());
        update_buttons();
    });
    connect(ui->goPrevButton, &QPushButton::clicked, [this](bool) {
        set_active(m_board_exporter->displayed()->prev_move());
        update_buttons();
    });
    connect(ui->aidCheckBox, &QCheckBox::toggled, [this](bool toggled) {
        m_board_exporter->set_visualize_connections(toggled);
        redraw();
    });
