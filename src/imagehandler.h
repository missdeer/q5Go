...
public:
    ...
    const QList<QPixmap> *getStonePixmaps() const
    {
        return &stonePixmaps;
    }
    const QList<QPixmap> *getGhostPixmaps() const
    {
        return &ghostPixmaps;
    }
    ...
    void set_white_color(const QColor &c)
    {
        m_w_col = c;
    }
    void set_black_color(const QColor &c)
    {
        m_b_col = c;
    }
...
private:
    ...
    void paint_stone_new(QImage &wi, int d, const QColor &, double, double, int, double, bool clamshell, int idx = 0);
