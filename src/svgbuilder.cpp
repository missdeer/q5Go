...
void svg_builder::fixed_height_text_at(
    double cx, double cy, double sidelen, const QString &txt, const QString &fill, const QFontInfo &fi, bool vcenter)
{
    int font_h = sidelen;
...
void svg_builder::circle_at(double cx, double cy, double r, const QString &fill, const QString &stroke, const QString &width)
{
...
{
    sidelen /= M_SQRT2 * 2;
    line(cx - sidelen, cy - sidelen, cx + sidelen, cy + sidelen, stroke, "2");
    line(cx + sidelen, cy - sidelen, cx - sidelen, cy + sidelen, stroke, "2");
}
